sap.ui.define(['jquery.sap.global', 'sap/ui/model/ChangeReason', 'sap/ui/model/ListBinding', './namespace'], function(
  jQuery,
  ChangeReason,
  ListBinding,
  namespace
  ...
  var MobxListBinding = ListBinding.extend(namespace + '.MobxListBinding', {
    constructor: function(model, path, context, sorters, filters, params) {
      ...
      this._mobxDisposer = mobx.reaction(function() {
        var observableArr = model.getProperty(path, context);
        if (observableArr && typeof observableArr.slice === 'function') return observableArr.slice();
